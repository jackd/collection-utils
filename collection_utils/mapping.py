...
import collections
from collection_utils.iterable import split


def _is_base_mapping(value):
    return isinstance(value, (dict, collections.Mapping))
    ...
    def __init__(self, **kwargs):
        for k, v in kwargs.items():
            if not _is_base_mapping(v):
                raise ValueError(
                    'expected Mapping for kwarg %s, got %s' % (k, v))
        self._children = kwargs
...
def flat_mapping(depth=1, **kwargs):
    if depth < 1:
        ...
        return NestedFlatMapping(
            **{k: v if isinstance(v, FlatMapping) else flat_mapping(
                depth=depth-1, **v) for k, v in kwargs.items()})
